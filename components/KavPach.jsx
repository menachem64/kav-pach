...
// ── PapaParse loader ─────────────────────────────────────────────────────────
let _papaLoaded = false;
const loadPapaParse = () => {
  if (_papaLoaded || window.Papa) { _papaLoaded = true; return Promise.resolve(); }
  return new Promise((res, rej) => {
    const src = "https://cdnjs.cloudflare.com/ajax/libs/PapaParse/5.4.1/papaparse.min.js";
    if (document.querySelector(`script[src="${src}"]`)) { _papaLoaded = true; return res(); }
    const s = document.createElement("script");
    s.src = src; s.onload = () => { _papaLoaded = true; res(); }; s.onerror = rej;
    document.head.appendChild(s);
  });
};

// ── שם קובץ ה-CSV המקומי (שנה לפי הצורך) ────────────────────────────────────
...
// ── פרסור שורות CSV/אקסל לפי headers ─────────────────────────────────────────
// מקבל מערך-של-אובייקטים (כמו PapaParse) וממיר לטריפים
const parseRowsToTrips = (rows) => {
  if (!rows || rows.length === 0) return { parsed: [], lineCitiesMap: new Map() };

  const headers = Object.keys(rows[0]);
  const colMatch = (...kws) => headers.find(h => kws.some(k => h.includes(k))) || null;
  ...
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r];
    const lineNum = String(cv(row, K.line)).trim();
    if (!lineNum || lineNum === "undefined") continue;

    const timeStr = fmtTime(cv(row, K.time));
    const mins = timeToMins(timeStr);
    if (mins === null) continue;

    const rideRaw = parseFloat(String(cv(row, K.ridership)).replace(/,/g, ""));
    const peakRaw = parseFloat(String(cv(row, K.peak)).replace(/,/g, ""));
    const ride = isNaN(rideRaw) ? 0 : rideRaw;
    const peak = isNaN(peakRaw) ? 0 : peakRaw;
    const daysInfo = parseDays(cv(row, K.days));

    const uniquenessVal = String(cv(row, K.uniqueness) || "");
    const isNight = uniquenessVal.includes("לילה");
    const isFeeding = uniquenessVal.includes("קווים מזינים") || uniquenessVal.includes("מזין");
    const makatVal = String(cv(row, K.makat) || "").trim();
    const opGroupVal = String(cv(row, K.opGroup) || "").trim();
    const originVal = String(cv(row, K.origin) || "").trim();
    const destVal = String(cv(row, K.dest) || "").trim();
    const isEilat = (originVal.includes("אילת") || destVal.includes("אילת")) && opGroupVal.includes("בינעירוני ארוך");

    const distanceRaw = parseFloat(String(cv(row, K.distance)).replace(/,/g, ""));
    ...
    parsed.push({
      ...
      lineNum,
      makat: makatVal,
      direction: String(cv(row, K.direction)).trim(),
      origin:    String(cv(row, K.origin) || "לא ידוע").trim(),
      dest:      String(cv(row, K.dest)   || "לא ידוע").trim(),
      time: timeStr, timeMins: mins, period: getPeriod(mins),
      days: daysInfo.text, daysList: daysInfo.list,
      district: String(cv(row, K.district) || "כללי").trim(),
      lineType: String(cv(row, K.lineType) || "עירוני").trim(),
  ...
  return { parsed, finalLineCitiesMap };
};
...
export default function KavPach() {
  ...
  // מצב אוטו-טעינה: null=לא נבדק, 'checking'=בודק, 'found'=נמצא, 'not-found'=לא נמצא
  const [autoLoadState, setAutoLoadState] = useState("checking");

  const [overlapLoad, setOverlapLoad] = useState({ active: false, progress: 0, message: "" });
  const setOverlapLoading = (active) => setOverlapLoad(s => ({ ...s, active }));
  const setOverlapProgress = (progress) => setOverlapLoad(s => ({ ...s, progress }));
  const setOverlapMessage = (message) => setOverlapLoad(s => ({ ...s, message }));
  ...
  const [overlapMode, setOverlapMode] = useState("cross");
  ...
  // ── ניסיון טעינה אוטומטית מ-CSV מקומי בהפעלת הרכיב ──────────────────────
  useEffect(() => {
    const tryAutoLoad = async () => {
      try {
        setAutoLoadState("checking");
        const resp = await fetch(LOCAL_CSV_PATH, { method: 'HEAD' });
        if (!resp.ok) { setAutoLoadState("not-found"); return; }

        // הקובץ קיים — טוען אותו
        setAutoLoadState("found");
        setFileLoading(true);
        setFileProgress(5);
        setFileMessage("מאתר קובץ CSV מקומי...");

        await loadPapaParse();
        setFileProgress(15);
        setFileMessage("קורא קובץ CSV...");
        await yieldFrame();

        const fullResp = await fetch(LOCAL_CSV_PATH);
        const text = await fullResp.text();
        setFileProgress(30);
        setFileMessage("מפרק CSV...");
        await yieldFrame();
        ...
        if (result.errors.length > 0) {
          console.warn("שגיאות CSV:", result.errors.slice(0, 5));
        }

        setFileProgress(50);
        setFileMessage(`מעבד ${result.data.length.toLocaleString()} שורות...`);
        await yieldFrame();
        ...
        for (let start = 0; start < rows.length; start += CHUNK) {
          ...
          const { parsed } = parseRowsToTrips(chunk);
          // תיקון id ייחודי
          parsed.forEach(t => { t.id = start + t.id; });
          allParsed.push(...parsed);

          const pct = 50 + Math.round(((start + CHUNK) / rows.length) * 45);
          setFileProgress(Math.min(pct, 97));
          setFileMessage(`נמצאו ${allParsed.length.toLocaleString()} נסיעות...`);
          await yieldFrame();
        }

        setTrips(allParsed);
        setFileProgress(100);
        setFileMessage(`נטענו ${allParsed.length.toLocaleString()} נסיעות מ-CSV ✓`);
        await yieldFrame();
        setFileLoading(false);

      } catch (err) {
        // CSV לא נמצא או לא נגיש — עוברים לממשק העלאה
        console.info("CSV מקומי לא זמין, ממתין להעלאת קובץ:", err.message);
        setAutoLoadState("not-found");
        setFileLoading(false);
      }
    };

    tryAutoLoad();
  }, []); // רץ פעם אחת בלבד בהפעלת הרכיב
  ...
  // ── העלאת קובץ ידנית (Excel) — נשאר כחלופה ──────────────────────────────
  ...
  const runOptimization = async (overrideLine, overrideCity, overrideDirection, overrideDays) => {
    ...
    for (let gi = 0; gi < groupEntries.length; gi++) {
      ...
      for(let i = 0; i < group.length; i++) {
        ...
        const getMetricVal = (t) => optMetric === 'peakLoad' ? t.peakLoad : t.ridership;
            ...
            if (allowCancel) {
              ...
              if (hasAlternative) {
                results.push({
                  ...
                  origin: t1.origin, dest: t1.dest,
                  direction: t1.direction,
                  time: t1.time, timeMins: t1.timeMins,
                  days: t1.days,
                  usedMetric: optMetric,
                  metricVal: valCancel,
  ...
  const handleOptimizeLine = (lineNum) => {
    setOptLine(lineNum);
    setOptCity("all");
    setOptDirection("all");
    setOptDays([]);
  ...
  const exportNetworkMasterPlanToExcel = () => {
    if (!filteredOverlap || filteredOverlap.length === 0) {
      alert("אין נתוני חפיפה לייצוא — הרץ את הניתוח קודם.");
      return;
    }
    ...
    for (const t of trips) {
      ...
      if (!lineStats.has(t.lineNum)) lineStats.set(t.lineNum, { riders: [], peaks: [], count: 0 });
      const s = lineStats.get(t.lineNum);
      s.riders.push(t.ridership || 0);
      s.peaks.push(t.peakLoad || 0);
      s.count++;
    }
    const avg = (arr) => arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : 0;

    const scoreMap = new Map();
    for (const l of redundantLines) scoreMap.set(l.lineNum, l.score);

    const getStats = (lineNum) => {
      const s = lineStats.get(lineNum);
      if (!s) return { avgRiders: 0, avgPeak: 0, score: 0, weeklyTrips: 0 };
      return {
        avgRiders: Math.round(avg(s.riders) * 10) / 10,
        avgPeak:   Math.round(avg(s.peaks)),
        score:     scoreMap.get(lineNum) || 0,
        weeklyTrips: s.count
      };
    };

    const rows = [];
    for (const r of filteredOverlap) {
      const sA = getStats(r.lineA);
      const sB = getStats(r.lineB);
      const segDesc = `${r.segment?.firstStop || ''} - ${r.segment?.lastStop || ''}`;

      let action, cancelled, absorber, justification;

      if (r.coverageA === 100 && r.coverageB === 100) {
        const weakIsA = sA.score >= sB.score;
        cancelled = weakIsA ? r.lineA : r.lineB;
        absorber  = weakIsA ? r.lineB : r.lineA;
        const wStats = weakIsA ? sA : sB;
        action = "ביטול כפילות";
        justification = `קו ${cancelled} וקו ${absorber} עוברים באותן ${r.segment?.length || ''} תחנות בדיוק. קו ${cancelled} חלש יותר (ציון אי-יעילות ${wStats.score}). המלצה: בטל קו ${cancelled} והעבר נוסעים לקו ${absorber}. חיסכון: ${wStats.weeklyTrips} נסיעות שבועיות.`;
      } else if (r.coverageA === 100 && r.coverageB < 100) {
        cancelled = r.lineA;
        absorber  = r.lineB;
        const capOk = sB.avgPeak < 40;
        action = capOk ? "ביטול חלופה קצרה" : "בדיקה נדרשת";
        justification = `קו ${r.lineA} מוכל לחלוטין בקו ${r.lineB} (${r.coverageA}%). ${capOk ? `לקו ${r.lineB} יש קיבולת פנויה (עומס שיא ממוצע ${sB.avgPeak}). ביטול קו ${r.lineA} יחסוך ${sA.weeklyTrips} נסיעות שבועיות.` : `אך לקו ${r.lineB} עומס גבוה (${sB.avgPeak}) — נדרשת בדיקה לפני ביטול.`}`;
      } else if (r.coverageB === 100 && r.coverageA < 100) {
        cancelled = r.lineB;
        absorber  = r.lineA;
        const capOk = sA.avgPeak < 40;
        action = capOk ? "ביטול חלופה קצרה" : "בדיקה נדרשת";
        justification = `קו ${r.lineB} מוכל לחלוטין בקו ${r.lineA} (${r.coverageB}%). ${capOk ? `לקו ${r.lineA} יש קיבולת פנויה (עומס שיא ממוצע ${sA.avgPeak}). ביטול קו ${r.lineB} יחסוך ${sB.weeklyTrips} נסיעות שבועיות.` : `אך לקו ${r.lineA} עומס גבוה (${sA.avgPeak}) — נדרשת בדיקה לפני ביטול.`}`;
      } else if (r.coverageA >= 85 && r.coverageB >= 85) {
        const weakIsA = sA.score >= sB.score;
        cancelled = weakIsA ? r.lineA : r.lineB;
        absorber  = weakIsA ? r.lineB : r.lineA;
        action = "תכנון מסלול מאוחד";
        justification = `קו ${r.lineA} וקו ${r.lineB} חופפים ב-${r.coverageA}%/${r.coverageB}%. ניתן לאחדם למסלול משותף אחד ולחסוך ${Math.min(sA.weeklyTrips, sB.weeklyTrips)} נסיעות שבועיות.`;
      } else {
        cancelled = r.lineA;
        absorber  = r.lineB;
        action = "חפיפה חלקית — לבדיקה";
        justification = `קווים ${r.lineA} ו-${r.lineB} חולקים מקטע של ${r.segment?.length || ''} תחנות (${r.pct}% חפיפה). מומלץ לבדוק אפשרות לייעול.`;
      }

      rows.push({
        'פעולה מומלצת': action,
        'קו לביטול / שינוי': cancelled,
        'קו קולט / חלופה': absorber,
        'אחוז חפיפה': `${r.pct}%`,
        'מקטע משותף': segDesc,
        'תחנות ברצף': r.segment?.length || '',
        'ממוצע נוסעים — קו מבוטל': cancelled === r.lineA ? sA.avgRiders : sB.avgRiders,
        'עומס שיא — קו קולט':      absorber  === r.lineA ? sA.avgPeak  : sB.avgPeak,
        'נסיעות שבועיות לחיסכון':  cancelled === r.lineA ? sA.weeklyTrips : sB.weeklyTrips,
        'הצדקה אנליטית': justification,
      });
    }

    const ws = window.XLSX.utils.json_to_sheet(rows);
    ws['!cols'] = [14,10,10,10,30,10,14,12,16,60].map(w => ({ wch: w }));
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, "תוכנית_התייעלות");
    window.XLSX.writeFile(wb, "תוכנית_התייעלות_רשתית.xlsx");
  };
  ...
  const exportOptimizationsToExcel = () => {
    ...
    const exportData = dataToExport.map(opt => {
      ...
      if (opt.type === 'merge') {
        return {
          'מספר קו': opt.line, 'סוג קו': opt.categoryLabel,
          'מוצא': opt.origin, 'יעד': opt.dest, 'כיוון': opt.direction,
          'ימי פעילות': opt.days, 'פעולה מומלצת': 'איחוד נסיעות',
        ...
        return {
          'מספר קו': opt.line, 'סוג קו': opt.categoryLabel,
          'מוצא': opt.origin, 'יעד': opt.dest, 'כיוון': opt.direction,
          'ימי פעילות': opt.days, 'פעולה מומלצת': 'ביטול נסיעה',
          'שעות מקוריות': opt.time, 'שעה מוצעת (חדשה)': '--',
        ...
        return {
          'מספר קו': opt.line, 'סוג קו': opt.categoryLabel,
          'מוצא': opt.origin, 'יעד': opt.dest, 'כיוון': opt.direction,
          'ימי פעילות': opt.days, 'פעולה מומלצת': 'ללא שינוי (תקין)',
          'שעות מקוריות': opt.time, 'שעה מוצעת (חדשה)': opt.time,
          'מדד (נוסעים / עומס)': `${metricName}: ${opt.metricVal}`,
  ...
  const redundantLines = useMemo(() => {
    ...
    return Object.entries(groups).map(([lineNum, data]) => {
      ...
      return {
        lineNum, avg: avgRiders.toFixed(1), count: totalTrips, score,
        origin: data[0].origin, dest: data[0].dest,
        district: data[0].district, makat: data[0].makat,
        status, percentLow: Math.round(percentLow), avgPeak: Math.round(avgPeak), wastedKm
      };
  ...
  const runOverlapAnalysis = async () => {
    ...
    try {
      ...
      const colMatch = (...kws) => headers.findIndex(h => kws.some(k => h.toLowerCase().includes(k.toLowerCase())));
      ...
      await yieldFrame();
      const cv = (r, cidx) => { if (cidx < 0) return ""; const cell = ws[enc({ r, c: cidx })]; return cell ? (cell.v ?? "") : ""; };
      ...
      const makatToDir     = new Map();
      trips.forEach(t => {
        if (t.makat && t.lineNum) makatToLineNum.set(String(t.makat).trim(), String(t.lineNum).trim());
      ...
      for (const [routeId, stops] of routeStops.entries()) {
        ...
        for (const s of stops) {
          if (!seen.has(s.stopId)) { seen.add(s.stopId); uniqueStops.push(s); }
        }
        ...
        routes.set(routeId, {
          ...
          stops: stopIds, stopNames, stopSet: new Set(stopIds),
      ...
      for (const key of candidatePairs) {
        ...
        for (let i = 1; i <= n; i++) {
          for (let j = 1; j <= mExt; j++) {
            if (a.stops[i-1] === extB[j-1]) {
              ...
              if (curr[j] > bestLen) { bestLen = curr[j]; bestEndA = i; bestEndB = j; }
            } else { curr[j] = 0; }
          }
          [prev, curr] = [curr, prev];
          curr.fill(0);
        ...
        const nameA = (idx) => a.stopNames[idx] || a.stops[idx] || `תחנה ${a.stops[idx]}`;
        const nameB = (idx) => b.stopNames[idx] || b.stops[idx] || `תחנה ${b.stops[idx]}`;
        const cityOf = (s) => s ? s.split('-')[0].trim() : s;

        const variantSuffix = (routeId) => {
          const parts = String(routeId).split('-');
        ...
        results.push({
          ...
          stopIdsA: a.stops, originA, destA,
          routeB: b.id, lineB: b.lineNum, makatB: b.makat, dirB: b.direction, stopsB: m,
          stopIdsB: b.stops, originB, destB,
          segment, segStartName: firstName, segEndName: lastName,
  ...
  const filteredOverlap = useMemo(() => {
    ...
    const cleanNum = s => String(s || '').replace(/^0+/, '').trim() || String(s || '').trim();
    const cityOnly = (s) => s ? (s.indexOf(' - ') > 0 ? s.slice(0, s.indexOf(' - ')).trim() : s.split('/')[0].trim()) : '';
    const getSig = (line, o, d) => `${cleanNum(line)}_${[cityOnly(o), cityOnly(d)].sort().join('-')}`;
    ...
    for (const r of deduped) {
      ...
      if (shortKey && multiGroups.has(shortKey)) {
        if (!usedInMulti.has(shortKey)) {
          ...
          const rawAbsorbers = group.map(g => {
          ...
          for (const ab of rawAbsorbers) {
            ...
            if (!seenAbsorbingSigs.has(abSig)) { seenAbsorbingSigs.add(abSig); uniqueAbsorbers.push(ab); }
          }
    ...
    const biMultiMap   = new Map();
    const biOverlapMap = new Map();
    ...
    for (const [shortSig, cards] of biMultiMap.entries()) {
      if (cards.length > 1) {
        ...
        cards.forEach(c => c._absorbers.forEach(ab => {
          const abSig = getSig(ab.line, ab.origin, ab.dest);
          if (!seenAbs.has(abSig)) { seenAbs.add(abSig); allAbsorbers.push(ab); }
        }));
    ...
    for (const [, cards] of biOverlapMap.entries()) {
      if (cards.length > 1) {
        const best = cards.reduce((a, b) => (b.pct > a.pct ? b : a));
        combinedFinal.push({ ...best, _biOverlap: true, _cards: cards, pct: Math.round(cards.reduce((s, c) => s + c.pct, 0) / cards.length) });
      } else {
        combinedFinal.push(cards[0]);
      }
    }
    ...
    if (overlapSearch.trim()) {
      ...
      if (searchVals.length > 0) {
        finalResult = finalResult.filter(r => {
          ...
          const matchMulti   = r._multi   && r._absorbers.some(ab => searchVals.includes(cleanNum(ab.line)));
          const matchBiMulti = r._biMulti && (searchVals.includes(cleanNum(r._shortLine)) || r._absorbers.some(ab => searchVals.includes(cleanNum(ab.line))));
          const matchBiOverlap = r._biOverlap && r._cards?.some(c => searchVals.includes(cleanNum(c.lineA)) || searchVals.includes(cleanNum(c.lineB)));
  ...
  // ── רינדור ───────────────────────────────────────────────────────────────
  const renderPrebookedInfo = (key, isEilat) => {
    if (!isEilat) return null;
    return (
      <span key={key} className="text-amber-500 bg-amber-50 px-1.5 py-0.5 rounded-full text-[10px] font-black border border-amber-200" title="קו בהזמנה מראש — נתוני תיקופים עשויים להיות חלקיים">
        🎫 הזמנה מראש
      </span>
    );
  };

  const renderFeedingLineInfo = (key, isFeeding) => {
    if (!isFeeding) return null;
    return (
      <span key={key} className="text-sky-600 bg-sky-50 px-1.5 py-0.5 rounded-full text-[10px] font-black border border-sky-200" title="קו מזין — יש להצליב עם לוח הרכבות">
        🚆 קו מזין
      </span>
    );
  };

  const renderTransitChip = (origin, dest) => null;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 p-4 md:p-6 pb-20" style={{ fontFamily: "'Heebo', sans-serif" }} dir="rtl">
      ...
      <div className="max-w-6xl mx-auto">
              ...
              <div className="space-y-8 text-slate-700 text-sm leading-relaxed">
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-2 marker:text-teal-400 pr-2">
                    ...
                    <li>
                      <strong>זיהוי קווים חכם:</strong> המערכת מציגה כעת התראות ואייקונים מיוחדים לקווים דורשי תשומת לב:
                      <ul className="list-none pr-6 mt-1 space-y-1 text-slate-600">
                        <li>🌙 קו לילה (מסומן באייקון ירח).</li>
                        <li>🎫 קו בהזמנה מראש (אילת) (כולל אזהרה שנתוני התיקופים עשויים להיות חסרים).</li>
                        <li>🚆 קו מזין רכבת (כולל המלצה להצליב את הנתונים מול לו"ז הרכבות טרם ביצוע שינויים).</li>
                      </ul>
                    </li>
                  </ul>
                </div>
        ...
        {/* ── מצב: בודק CSV ─────────────────────────────────────────────────── */}
        {autoLoadState === "checking" && !fileLoad.active && trips.length === 0 && (
          <div className="flex flex-col items-center justify-center py-40 text-center gap-4">
            <Ic n="loader" size={48} cls="text-slate-400" animate={true} />
            <p className="text-slate-500 font-bold">בודק קובץ נתונים מקומי...</p>
          </div>
        )}

        {/* ── מצב: טוען קובץ (CSV אוטו או Excel ידני) ─────────────────────── */}
        {fileLoad.active && (
        ...
        {/* ── מצב: CSV לא נמצא ← מציג ממשק העלאה ──────────────────────────── */}
        {!fileLoad.active && trips.length === 0 && autoLoadState === "not-found" && (
          <div className="flex flex-col items-center justify-center py-32 px-6 bg-white rounded-[3rem] border-4 border-dashed border-slate-200 shadow-sm text-center">
            ...
            <h3 className="text-xl font-black mb-3 bg-indigo-50 text-indigo-800 px-5 py-2 rounded-xl border border-indigo-100 shadow-sm inline-block">המערכת שמוצאת קווים שאפשר לזרוק לפח</h3>
            <p className="text-slate-500 font-medium mb-4 max-w-md">
              העלו קובץ אקסל עם נתוני תיקופים כדי להתחיל בניתוח המערכת.
            </p>
            {/* הסבר על CSV אוטומטי */}
            <div className="mb-8 bg-teal-50 border border-teal-100 text-teal-700 text-xs font-bold px-5 py-3 rounded-2xl max-w-sm">
              💡 טיפ: הניחו קובץ בשם <code className="font-mono bg-white px-1.5 py-0.5 rounded border border-teal-200">/data.csv</code> בשרת כדי שהנתונים ייטענו אוטומטית בכל פתיחה.
            </div>
        ...
        {/* ── מצב: נתונים נטענו ← מציג ממשק ראשי ──────────────────────────── */}
        {!fileLoad.active && trips.length > 0 && (
          <main>
            {/* כפתור החלפת קובץ */}
            <div className="flex justify-end mb-4">
              <label className="flex items-center gap-2 bg-white border-2 border-slate-200 text-slate-500 hover:border-slate-900 hover:text-slate-900 px-4 py-2 rounded-2xl font-black text-xs cursor-pointer transition-all shadow-sm">
                <Ic n="upload" size={14} />
                החלף קובץ נתונים
                <input type="file" className="hidden" accept=".xlsx,.xls" onChange={onFile} />
              </label>
            ...
            <nav className="flex bg-slate-200/50 backdrop-blur p-1.5 rounded-[2rem] mb-12 max-w-3xl mx-auto shadow-inner border border-slate-200">
              ...
              <div className="space-y-8">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                  <div>
                    ...
                    <p className="text-slate-500 font-bold">דירוג המציג את הקווים החלשים ביותר במערכת</p>
                  </div>
                  <div className="flex flex-col md:flex-row gap-3">
                    <select value={filterDistrict} onChange={e => setFilterDistrict(e.target.value)} className="bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3 font-black outline-none focus:border-slate-900 text-right shadow-sm w-full md:w-48 appearance-none cursor-pointer">
                    ...
                    <div className="flex relative w-full md:w-64">
                      ...
                      <input type="text" list="cities-list" value={searchCity} onChange={e => setSearchCity(e.target.value)} placeholder="הקלד עיר לחיפוש..." className="bg-slate-50 border-2 border-slate-200 rounded-2xl pr-12 pl-6 py-3 font-black outline-none focus:border-slate-900 text-right shadow-sm w-full" />
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredRedundant.length > 0 ? filteredRedundant.map(res => (
                    <div key={res.lineNum} className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-7 shadow-sm hover:border-slate-900 transition-all text-right flex flex-col">
                      <div className="flex items-start justify-between mb-6">
                        <div className="flex flex-col gap-2 items-start">
                          <div className={`px-4 py-1.5 rounded-full text-[11px] font-black border ${res.score >= 80 ? "bg-rose-50 border-rose-200 text-rose-600" : "bg-amber-50 border-amber-200 text-amber-700"}`}>{res.status}</div>
                          <div className="mt-1"><RouteFormat val={res.makat} /></div>
                        </div>
                      ...
                      <div className="flex-1 mb-5">
                        ...
                        <div className="space-y-2.5 pt-4 border-t border-slate-100">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-slate-600 font-bold">קילומטר מבוזבז בשבוע</span>
                ...
                <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex relative w-full md:w-auto">
                      <div className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"><Ic n="search" size={18}/></div>
                      <input type="text" list="cities-list" value={searchCity} onChange={e => setSearchCity(e.target.value)} placeholder="חיפוש עיר..." className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl pr-12 pl-4 py-3 font-black outline-none focus:border-slate-900 text-right shadow-sm" />
                    </div>
                  </div>
                </header>
                        ...
                        <th className="p-5">
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex flex-col -space-y-1.5 mr-2">
                              <button onClick={() => setSortConfig({key: 'ridership', direction: 'desc'})} className={`${sortConfig.key === 'ridership' && sortConfig.direction === 'desc' ? 'text-indigo-600' : 'text-slate-300'}`}><Ic n="chevronUp" size={12} strokeWidth="3" /></button>
                              <button onClick={() => setSortConfig({key: 'ridership', direction: 'asc'})} className={`${sortConfig.key === 'ridership' && sortConfig.direction === 'asc' ? 'text-indigo-600' : 'text-slate-300'}`}><Ic n="chevronDown" size={12} strokeWidth="3" /></button>
                            </div>
                          </div>
                        </th>
                        <th className="p-5">
                          <div className="flex items-center gap-1.5">
                            <span>עומס שיא</span>
                            <div className="flex flex-col -space-y-1.5 mr-2">
                              <button onClick={() => setSortConfig({key: 'peakLoad', direction: 'desc'})} className={`${sortConfig.key === 'peakLoad' && sortConfig.direction === 'desc' ? 'text-indigo-600' : 'text-slate-300'}`}><Ic n="chevronUp" size={12} strokeWidth="3" /></button>
                              <button onClick={() => setSortConfig({key: 'peakLoad', direction: 'asc'})} className={`${sortConfig.key === 'peakLoad' && sortConfig.direction === 'asc' ? 'text-indigo-600' : 'text-slate-300'}`}><Ic n="chevronDown" size={12} strokeWidth="3" /></button>
                            </div>
                          </div>
                        </th>
                      </tr>
                        ...
                        <tr key={i} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                          <td className="p-5 font-black">
                            <div className="flex items-center gap-2 justify-start">
                              ...
                              {t.isNightLine && <span className="text-indigo-400 bg-indigo-50 p-1 rounded-full" title="קו לילה"><Ic n="moon" size={16} /></span>}
                          ...
                          <td className={`p-5 ${t.ridership >= 40 ? 'text-rose-600 font-black' : ''}`}>{t.ridership}</td>
            ...
            {tab === "simulator" && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm max-w-4xl mx-auto">
                <header className="mb-8">
                  <h2 className="text-2xl font-black text-slate-900 mb-2">אלגוריתם ייעול ושיפור לוחות זמנים</h2>
                  <p className="text-slate-500 font-bold text-sm leading-relaxed">המערכת מזהה אוטומטית אם הקו הוא עירוני, אזורי או בין-עירוני ומפעילה חוקי איחוד וביטול שונים בהתאם.</p>
                </header>
                ...
                <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 mb-8">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                    <div>
                      ...
                      <input type="text" value={optLine} onChange={e => setOptLine(e.target.value)} placeholder="למשל 150, 10102..." className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-2.5 font-black text-sm outline-none focus:border-slate-900 shadow-sm transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-black text-slate-400 mb-2 pr-2">עיר (מוצא או יעד)</label>
                      <input type="text" list="cities-list" value={optCity === "all" ? "" : optCity} onChange={e => setOptCity(e.target.value || "all")} placeholder="למשל: אופקים ..." className="w-full bg-white border-2 border-slate-200 rounded-2xl px-4 py-3 font-black outline-none focus:border-slate-900 text-right transition-all shadow-sm" />
                    </div>
                    <div>
                  ...
                  <div className="mb-6">
                    <label className="block text-xs font-black text-slate-400 mb-2 pr-2">ימי פעילות</label>
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => setOptDays([])} className={`px-4 py-2 rounded-2xl text-sm font-black transition-all ${optDays.length === 0 ? 'bg-teal-600 text-white shadow-md border-2 border-teal-600' : 'bg-white border-2 border-slate-200 text-slate-500 hover:border-teal-600'}`}>כל הימים</button>
                      {DAYS_FILTER.map(d => (
                        <button key={d.id} onClick={() => toggleDay(d.id)} className={`px-4 py-2 rounded-2xl text-sm font-black transition-all ${optDays.includes(d.id) ? 'bg-teal-600 text-white shadow-md border-2 border-teal-600' : 'bg-white border-2 border-slate-200 text-slate-500 hover:border-teal-600'}`}>{d.label}</button>
                      ))}
                    </div>
                  </div>

                  <div className="border-t border-slate-200/60 pt-4 mb-2">
                    <button onClick={() => setShowAdvanced(prev => !prev)} className="flex items-center gap-2 text-xs font-black text-slate-400 hover:text-slate-700 transition-colors">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transition: 'transform 0.2s', transform: showAdvanced ? 'rotate(180deg)' : 'rotate(0deg)' }}><path d="M19 9l-7 7-7-7"/></svg>
                      הגדרות מתקדמות
                    </button>
                    ...
                    {showAdvanced && (
                      <div className="flex flex-wrap gap-4 mt-4 items-end">
                        <div>
                          <label className="block text-xs font-black text-slate-400 mb-1.5">מדד לניתוח</label>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {[["15","15"],["30","30"],["60","60"],["120","120"],["180","180"]].map(([label,val]) => (
                              <button key={label} onClick={() => setOptCustomGap(optCustomGap === val ? "" : val)} className={`px-2.5 py-1 rounded-lg text-xs font-black transition-all ${optCustomGap === val ? 'bg-teal-600 text-white border-2 border-teal-600' : 'bg-white border-2 border-slate-200 text-slate-500 hover:border-teal-600'}`}>{label}</button>
                            ))}
                            <input type="number" min="1" max="1440" value={optCustomGap} onChange={e => { const v = e.target.value; if (v === "" || (parseInt(v) > 0 && parseInt(v) <= 1440)) setOptCustomGap(v); }} placeholder="אחר..." className="w-20 bg-white border-2 border-slate-200 rounded-xl px-2 py-1 font-black text-xs outline-none focus:border-slate-900 text-right shadow-sm" />
                          </div>
                        </div>
                        <div>
                          <label className="block text-xs font-black text-slate-400 mb-1.5">מינימום נסיעות להשאיר (ביום)</label>
                          <input type="number" min="0" value={optMinTrips} onChange={e => setOptMinTrips(e.target.value)} placeholder="למשל: 4..." className="w-32 bg-white border-2 border-slate-200 rounded-xl px-3 py-2 font-black text-xs outline-none focus:border-slate-900 text-right shadow-sm" />
                        </div>
                        <div>
                          <label className="block text-xs font-black text-slate-400 mb-1.5">רף נוסעים לביטול נסיעה</label>
                          <input type="number" min="0" step="0.1" value={optCancelThreshold} onChange={e => setOptCancelThreshold(e.target.value)} placeholder="ברירת מחדל: 5..." className="w-32 bg-white border-2 border-slate-200 rounded-xl px-3 py-2 font-black text-xs outline-none focus:border-slate-900 text-right shadow-sm" />
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-200/60">
                    <button onClick={() => runOptimization()} className="bg-teal-600 hover:bg-teal-700 text-white px-8 py-3.5 rounded-2xl font-black transition-all shadow-md active:scale-95 flex items-center gap-2">
                    ...
                    {optimizations.length > 0 && (
                      <button onClick={exportOptimizationsToExcel} className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3.5 rounded-2xl font-black text-sm transition-all shadow-md flex items-center gap-2">
                        <Ic n="download" size={18} /> ייצוא לאקסל
                      </button>
                    )}
                  </div>
                </div>
                ...
                {optimizations.length > 0 && (
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4 border-b border-slate-200 pb-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-sm font-bold">נמצאו {optimizations.filter(o => o.type !== 'ok').length} המלצות לשינויים</p>
                    </div>
                    <label className="flex items-center gap-2 bg-slate-100 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-slate-200 transition-colors">
                      <input type="checkbox" checked={showAllTripsInSimulator} onChange={e => setShowAllTripsInSimulator(e.target.checked)} className="w-4 h-4 accent-indigo-600 rounded" />
                      ...
                      <>
                            ...
                            <div key={`opt-${i}`} className="bg-white border-2 border-slate-50 p-6 rounded-[2rem] flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:shadow-lg transition-all border-r-4 border-r-indigo-500">
                              <div className="flex items-start gap-4">
                                ...
                                <div>
                                  <div className="flex items-center gap-2 mb-1.5">
                                    ...
                                    {opt.isNightLine && <span className="text-indigo-400 bg-indigo-50 p-1 rounded-full" title="קו לילה"><Ic n="moon" size={16} /></span>}
                              ...
                              <div className="bg-slate-50/80 px-6 py-4 rounded-2xl flex-1 max-w-md w-full">
                                <div className="flex justify-between items-center mb-3 text-sm">
                                  ...
                                  <span className="font-black text-slate-700">{opt.from} ו-{opt.to} <span className="text-xs text-slate-400 font-normal">({opt.gap} דק&apos;)</span></span>
                                </div>
                                <div className="flex justify-between items-center mb-4 text-sm">
                                  <span className="font-bold text-slate-500">{opt.usedMetric === 'peakLoad' ? 'עומס שיא מצטבר:' : 'נוסעים מצטבר:'}</span>
                                  <span className="font-black text-slate-700">{opt.total} <span className="text-xs text-slate-400 font-normal">({opt.val1} / {opt.val2})</span></span>
                                </div>
                                <div className="pt-3 border-t border-slate-200 flex justify-between items-center">
                                  <span className="font-black text-indigo-700">שעה מומלצת:</span>
                            ...
                            <div key={`opt-${i}`} className={`bg-white border-2 border-slate-50 p-6 rounded-[2rem] flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:shadow-lg transition-all border-r-4 ${opt.isTrash ? 'border-r-red-600 bg-red-50/20' : 'border-r-rose-500'}`}>
                              <div className="flex items-start gap-4">
                                ...
                                <div>
                                  <div className="flex items-center gap-2 mb-1.5">
                                    <span className="font-black text-slate-900 text-lg">קו {opt.line}</span>
                                    {opt.isNightLine && <span className="text-indigo-400 bg-indigo-50 p-1 rounded-full"><Ic n="moon" size={16} /></span>}
                                    {renderPrebookedInfo('sim-'+i, opt.isEilatPrebooked)}
                                    {renderFeedingLineInfo('feed-'+i, opt.isFeedingLine)}
                                    <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded font-bold">{opt.categoryLabel}</span>
                                  </div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    <span className="text-[11px] font-black bg-slate-100 text-slate-500 px-2 py-1 rounded-md">יום {opt.days}</span>
                                    <span className={`text-[11px] font-black px-2 py-1 rounded-md ${opt.isTrash ? 'bg-red-100 text-red-700' : 'bg-rose-100 text-rose-700'}`}>{opt.isTrash ? 'נסיעה כמעט ריקה !' : 'חשד לנסיעה מיותרת'}</span>
                                    <span className="text-[11px] font-black bg-sky-100 text-sky-700 px-2 py-1 rounded-md">כיוון {opt.direction}</span>
                                  </div>
                                </div>
                              </div>
                              <div className="bg-slate-50/80 px-6 py-4 rounded-2xl flex-1 max-w-md w-full">
                                ...
                                <div className="flex justify-between items-center text-sm pt-3 border-t border-slate-200">
                                  ...
                                  <span className={`font-black ${opt.isTrash ? 'text-red-600' : 'text-rose-600'}`}>{opt.efficiency}</span>
                                </div>
                              </div>
                            </div>
                          ) : (
                              ...
                              <div className="flex items-start gap-4">
                                ...
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    ...
                                    <span className="text-[11px] font-black bg-emerald-100 text-emerald-700 px-2 py-1 rounded-md">נסיעה תקינה</span>
                                  </div>
                                </div>
                              </div>
                        ...
                        {optsToRender.length > visibleOptCount && (
                          <div className="pt-4 text-center">
                            <button onClick={() => setVisibleOptCount(prev => prev + 50)} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black text-sm transition-all shadow-md flex items-center justify-center gap-2">
                              <Ic n="chevronDown" size={18} />
                              הצג עוד תוצאות
                              <span className="bg-indigo-500 text-white text-xs px-2.5 py-1 rounded-full font-black">{visibleOptCount} / {optsToRender.length.toLocaleString()}</span>
                            </button>
                          </div>
                        )}
                      </>
            ...
            {tab === "overlap" && (
              <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm max-w-5xl mx-auto">
                <header className="mb-8">
                  <h2 className="text-2xl font-black text-slate-900 mb-2">חפיפת מסלולים</h2>
                  <p className="text-slate-500 font-bold text-sm leading-relaxed">
                    זיהוי קווים עם מסלול דומה.
                    {!stopsReady && <span className="text-amber-600 mr-2">⚠️ כדי להפעיל את הניתוח יש להעלות את קובץ האקסל המלא (שכולל גיליון תחנות).</span>}
                  </p>
                </header>
                ...
                <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 mb-8">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                    <div>
                      ...
                      <input type="text" value={overlapSearch} onChange={e => setOverlapSearch(e.target.value)} placeholder="למשל 150, 10102..." className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-2.5 font-black text-sm outline-none focus:border-slate-900 shadow-sm transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-black text-slate-400 mb-2">עיר (מוצא או יעד)</label>
                      <input type="text" value={overlapCity} onChange={e => setOverlapCity(e.target.value)} placeholder="למשל: אופקים ..." list="cities-list" className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3 font-black outline-none focus:border-slate-900 text-right shadow-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-black text-slate-400 mb-2">מחוז</label>
                      <select value={overlapDistrict} onChange={e => setOverlapDistrict(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3 font-black outline-none focus:border-slate-900 text-right shadow-sm appearance-none cursor-pointer">
                        <option value="all">כל המחוזות</option>
                        {allDistricts.map(d => <option key={d} value={d}>{d}</option>)}
                      </select>
                    </div>
                  ...
                  <div className="border-t border-slate-200/60 pt-4 mb-2">
                    <button onClick={() => setShowAdvancedOverlap(v => !v)} className="flex items-center gap-2 text-xs font-black text-slate-500 hover:text-slate-900 transition-colors mb-3">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transition: 'transform 0.2s', transform: showAdvancedOverlap ? 'rotate(180deg)' : 'rotate(0deg)' }}><path d="M19 9l-7 7-7-7"/></svg>
                      הגדרות מתקדמות
                    </button>
                    {showAdvancedOverlap && (
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            {[[70,"70%"],[80,"80%"],[85,"85%"],[90,"90%"]].map(([val,label]) => (
                              <button key={val} onClick={() => setOverlapThreshold(val)} className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all ${overlapThreshold === val ? 'bg-teal-600 text-white' : 'bg-white border-2 border-slate-200 text-slate-500 hover:border-teal-600'}`}>{label}</button>
                            ))}
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="flex gap-2">
                            {[["cross","בין קווים שונים"],["same","חלופות באותו קו"],["all","הכל"]].map(([mode,label]) => (
                              <button key={mode} onClick={() => setOverlapMode(mode)} className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all ${overlapMode === mode ? 'bg-teal-600 text-white' : 'bg-white border-2 border-slate-200 text-slate-500 hover:border-teal-600'}`}>{label}</button>
                            ))}
                          </div>
                        </div>
                        <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-200/60">
                    {stopsReady ? (
                      <button onClick={runOverlapAnalysis} className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3.5 rounded-2xl font-black text-sm transition-all shadow-md flex items-center gap-2">
                        <Ic n="zap" size={18} /> הרץ ניתוח חפיפה
                      </button>
                    ) : (
                      <div className="flex items-center gap-3 text-sm font-bold text-amber-600 bg-amber-50 border border-amber-200 px-4 py-2.5 rounded-xl">
                        <Ic n="alert" size={16} />
                        יש להעלות את קובץ האקסל המלא (עם גיליון תחנות) כדי להפעיל ניתוח זה
                      </div>
                    )}
                    {filteredOverlap.length > 0 && (
                      <button onClick={exportNetworkMasterPlanToExcel} className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-3.5 rounded-2xl font-black text-sm transition-all shadow-md flex items-center gap-2 border-none">
                        <Ic n="download" size={18} /> הפק תוכנית התייעלות
                      </button>
                    )}
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <div className="space-y-3">
                      {filteredOverlap.slice(0, 500).map((r, i) => {
                        const getVariant = (routeId) => {
                          const parts = String(routeId || '').split('-');
                          const v = parts.length > 2 ? parts[parts.length - 1] : null;
                          return (v && v !== '0' && v !== '#') ? v : null;
                        };

                        if (r._biMulti || r._multi || r._biOverlap) {
                          // simplified rendering for multi/biMulti/biOverlap cards
                          const label = r._biMulti
                            ? `קו ${r._shortLine} מוכל במלואו (שני כיוונים)`
                            : r._multi
                            ? `קו ${r._shortLine} מוכל במלואו`
                            : `קו ${r.lineA} ↔ קו ${r.lineB} — חפיפה דו-כיוונית`;
                          const borderColor = r._biMulti ? 'border-r-amber-500' : r._multi ? 'border-r-orange-400' : 'border-r-teal-500';
                          const badgeColor = r._biMulti ? 'bg-amber-100 text-amber-800' : r._multi ? 'bg-orange-100 text-orange-700' : 'bg-teal-100 text-teal-800';
                          return (
                            <div key={i} className={`bg-white border border-slate-100 border-r-4 ${borderColor} rounded-2xl p-5 hover:shadow-md transition-shadow`}>
                              <div className="flex items-center justify-between mb-3">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${badgeColor}`}>{label}</span>
                                <span className="text-sm font-bold text-slate-500">{r.pct}%</span>
                              </div>
                              ...
                              {r._absorbers && (
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {r._absorbers.map((ab, ai) => (
                                    <span key={ai} className="bg-slate-100 text-slate-600 text-xs font-bold px-2 py-1 rounded-lg">קו {ab.line}</span>
                                  ))}
                                </div>
                              )}
                            </div>
                        ...
                        return (
                          <div key={i} className={`bg-white border border-slate-100 border-r-4 ${border} rounded-2xl p-5 hover:shadow-md transition-shadow`}>
                            <div className="flex items-center justify-between mb-4">
                              ...
                              <span className="text-sm font-bold text-slate-500">{r.pct}% כיסוי מקסימלי</span>
                            </div>
                              ...
                              {[
                                ...
                                <div key={qi} className="bg-slate-50 rounded-xl p-3">
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {lineToIsNight.get(String(q.line)) && <span className="text-indigo-400 bg-indigo-50 p-0.5 rounded-full" title="קו לילה"><Ic n="moon" size={14} /></span>}
                                  ...
                                  <div className="mt-2"><RouteFormat val={q.route} /></div>
                                </div>
                              ))}
                            </div>
                            <div className="bg-slate-50 rounded-xl p-3">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-bold text-slate-800">מקטע משותף — {r.segment.length} תחנות ברצף</span>
                                {!r.isCircular && (
                                  <button onClick={() => setActiveExplanation(activeExplanation === i ? null : i)} className={`text-xs px-2.5 py-1 rounded-lg font-bold transition-all ${activeExplanation === i ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:border-slate-400'}`}>
                              ...
                              {activeExplanation === i && (
                                <div className="mt-3 p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-sm leading-relaxed text-indigo-900">
                                  {r.explanation}
                                </div>
                              )}
                            </div>
                      ...
                      {filteredOverlap.length > 500 && (
                        <div className="text-center py-3 text-xs font-bold text-slate-400">מציג 500 מתוך {filteredOverlap.length.toLocaleString()} תוצאות</div>
                      )}
                    </div>
                  </>
                )}
              ...
              <div className="bg-white p-8 md:p-12 rounded-[3rem] border border-slate-200 shadow-sm max-w-4xl mx-auto transition-opacity duration-300 opacity-100">
                ...
                <div className="space-y-10">
                  <section className="bg-indigo-50 rounded-[2rem] p-6 border border-indigo-100">
                    <h3 className="text-xl font-black text-indigo-700 mb-4">מה חדש?</h3>
                    <div className="space-y-5 text-slate-700 font-medium leading-relaxed">
                      <div>
                        <div className="font-black text-slate-900 mb-2">פיצ'רים חדשים</div>
                        <ul className="list-none space-y-1.5 text-sm pr-2">
                          <li>• <strong>חפיפת מסלולים:</strong> טאב חדש שמזהה אוטומטית קווים עם מסלול זהה או חופף, מבוסס על איתור רצפים רצופים של תחנות.</li>
                          <li>• זיהוי קווי לילה, קווי הזמנה מראש וקווים מזינים — עם הסבר מובנה על השפעתם על נתוני התיקופים.</li>
                          <li>• ייצוא "תוכנית התייעלות רשתית" לאקסל ישירות מטאב החפיפות.</li>
                          <li>• חיפוש מרובה קווים בו-זמנית, מופרדים בפסיק: למשל "140, 160, 200".</li>
                          <li>• הגדרות מתקדמות מוסתרות בסימולטור: מרווח לאיחוד, רף ביטול ומינימום נסיעות — ניתנים לכוונון ידני.</li>
                          <li>• כפתורי בחירה מהירה למרווח איחוד: 15 / 30 / 60 / 120 / 180 דקות.</li>
                          <li>• אפשרות סינון לולאות מעגליות בטאב החפיפות.</li>
                        </ul>
                      </div>
                      <div>
                        <div className="font-black text-slate-900 mb-2">תיקוני באגים ושיפורי ביצועים</div>
                        <ul className="list-none space-y-1.5 text-sm pr-2">
                          <li>• פתרון לבעיית קפיאת האתר בקבצים גדולים: עיבוד במנות של 3,000–5,000 שורות עם הפסקות קצרות בין כל מנה.</li>
                          <li>• תיקון פענוח שעה 00:00 שלא זוהתה נכון.</li>
                          <li>• תיקון כפילויות בתוצאות החפיפה — כל זוג קווים מוצג פעם אחת בלבד.</li>
                          <li>• תיקון קריסה בהרצת האלגוריתם על קובץ גדול ללא סינון.</li>
                        </ul>
                      </div>
                    </div>
                  </section>
